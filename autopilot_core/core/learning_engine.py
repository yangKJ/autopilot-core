...
class LearningEngine:
    ...
    def _get_default_db_path(self) -> Path:
        """获取默认数据库路径"""
        return Path(".autopilot") / ".learning.db"
    ...
    def get_recommended_skills(self, files: List[str], base_skills: List[str]) -> List[str]:
        ...
        if high_risk:
            if "self-healing" not in recommended:
                recommended.insert(0, "self-healing")
...
def get_learning_engine(db_path: Path = None) -> LearningEngine:
    """获取学习引擎单例"""
...
def get_recommended_skills_for_files(files: List[str], base_skills: List[str] = None) -> List[str]:
    """获取文件的推荐技能"""
    engine = get_learning_engine()
    return engine.get_recommended_skills(files, base_skills or [])
